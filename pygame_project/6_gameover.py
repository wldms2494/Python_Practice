...
# 2. 이벤트 처리 부분(키보드, 마우스 등)
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP: # 키보드에서 손 땠을때
...
# 3. 게임 케릭터 위치 정의
    ...
    # 무기 위치 조정
    # 100, 200 -> 쏘고 나면 x좌표 100은 그대로 남고, 200 - 180 - 160 - 140... 줄어든다
    weapons = [[w[0], w[1] - weapon_speed] for w in weapons] #weapons의 값 하나하나를 w에 담아서 높이는 speed만큼 빼서 weapon을 위로 쏘는 과정
    ...
    for ball_idx, ball_val in enumerate(balls): # enumerate : index 몇번째인지 알려줌
        ...
        if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width:
            ball_val["to_x"] = ball_val["to_x"] * -1 # 가로벽에 닿았을 때 공 이동위치를 반대로 변경(튕기는 효과)
        # 세로 위치 
        if ball_pos_y > screen_height-stage_height - ball_height : # 공이 스테이지에 닿았을때
    ...
    for ball_idx, ball_val in enumerate(balls): # enumerate : index 몇번째인지 알려줌
        ...
        for weapon_idx, weapon_val in enumerate(weapons) :
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                ball_to_remove = ball_idx # 해당 공 없애기 위한 값 설정 => 부딪히면 바로 없애는게 아니라 큰공이라면 큰 공은 사라지고 작은 공으로 쪼개져야함.

                # 가장 작은 크기의 공 (4번째)가 아니라면 다음 단계의 공으로 나눠주기
                if ball_img_idx <3: 
                    # 현재 공 크기 정보를 가지고 옴.
